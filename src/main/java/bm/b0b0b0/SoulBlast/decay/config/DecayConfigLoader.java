...
public final class DecayConfigLoader {

    public static final String FOLDER = "decay";

    private final JavaPlugin plugin;

    public DecayConfigLoader(JavaPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public <T extends net.elytrium.serializer.language.object.YamlSerializable> T load(
        ...
        if (!Files.exists(path)) {
            if (defaults instanceof DecayBlocksFileConfig) {
                DecayConfigBootstrap.writeBlocks(path);
            } else {
    ...
    private void ensureParent(Path path) {
        try {
            ...
            throw new IllegalStateException("Cannot create decay folder for " + path, exception);
        }
    }
