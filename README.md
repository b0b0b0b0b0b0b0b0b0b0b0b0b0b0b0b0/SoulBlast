...
## CoreProtect (откат взрывов)

**Мягкая зависимость** — в `plugin.yml` указан `softdepend: CoreProtect`. CP не обязателен: SoulBlast грузится и без него. Если **CoreProtect 22+** на сервере и в `config.yml` включена интеграция, все изменения блоков от взрывов пишутся в CP под псевдо-игроком **`#soulblast`** (не под ником поджигателя).

**Автоматически для всех зарядов** — и для шести пресетов, и для **кастомных бомб админа** в `dynamites/dynamites.yml`. Привязка не по `id`, а к **любому взрыву** через общую очередь `ExplosionJob`: добавил новый заряд — лог и откат работают сами, если интеграция включена.

**Что логируется:** сломанные и поставленные блоки, осушение воды/лавы, огонь, TNT-блоки, мгновенный break через Decay при взрыве, разрушение PS-блока привата. **Не логируется:** постепенный Decay по тикам после рейда, ванильный fire spread.

### Откат

Встань в **центр кратера** и выполни:

```
/soulblast rollback #soulblast <радиус> <время>
```

| Параметр | Пример | Смысл |
|----------|--------|--------|
| пользователь | `#soulblast` | всегда этот псевдоним (см. `fallback-user` в конфиге) |
| радиус | `80` | блоки от твоей позиции |
| время | `30m`, `1h`, `3600` | как далеко назад откатывать |

Право: `soulblast.rollback` (по умолчанию op). Откат идёт **асинхронно** через API CoreProtect.

**Проверка перед откатом** (команды CoreProtect, не SoulBlast):

```
/co lookup r:50 t:5m u:#soulblast a:block #count
/co lookup r:50 t:5m u:#soulblast a:-block i:water #count
```

Фильтр `a:block` — только блоки; без него в выдачу попадёт подбор предметов с твоего ника. Для орешника бери радиус и время с запасом (`80–100`, `1h`).

Настройки: `config.yml` → `core-protect-integration` (`enabled`, `log-breaks`, `log-placements`, `log-liquid-clear`, `fallback-user`, `log-all-as-fallback-user`).
...
- **CoreProtect** 22+ (опционально, **softdepend**) — лог взрывов SoulBlast и откат кратера; без CP плагин стартует как обычно, просто нет `/soulblast rollback` и записей в базе CP
...
| `/soulblast rollback <user> <радиус> <время>` | Откат взрыва SoulBlast через CoreProtect (нужен CP 22+). Пример: `/soulblast rollback #soulblast 80 1h` — стоя в центре кратера |
...
|-------|----------|----------------|
| `soulblast.use` | доступ к `/soulblast` | op |
| `soulblast.give` | выдача динамитов (`/soulblast give`) | op |
| `soulblast.menu` | гримуар (`/soulgrimoire`, `/soulblast menu`) | все |
| `soulblast.reload` | перезагрузка конфигов (`/soulblast reload`) | op |
| `soulblast.rollback` | откат взрывов SoulBlast (`/soulblast rollback`, нужен CoreProtect) | op |
| `soulblast.region.bypass` | обход WG-зон (не действует на `region-names` при `strict-listed-regions`) | выкл |
| `soulblast.cooldown.bypass` | обход кулдауна между зарядами | op |
| `soulblast.decay.menu` | каталог блоков Decay (`/decayblocks`) | все |
| `soulblast.decay.repair` | ручная починка треснутых стен (песок) | все |
| `soulblast.ps.hologram` | `/psholo` для **своего** привата | все |
| `soulblast.ps.hologram.admin` | `/psholo` на любом PS-блоке | op |
...
| Файл | Что внутри |
|------|------------|
| [`config.yml`](src/main/resources/config.yml) | взрывы, лимиты, экономика, WG, PS, CoreProtect, кулдауны, сущности |
